...
function makeWorkspace(detail, pass, distances, relaxations, edge, weight) {
    return {
...
export function createBellmanFordInitialStep(graph) {
    return {
        type: "start",
        graph,
        current: graph.start,
...
export function* bellmanFord(graph) {
        ...
        for (const current of cells) {
            ...
            for (const neighbor of neighborsFor(graph, current)) {
                ...
                yield {
                    ...
                    workspace: makeWorkspace(`Compare ${label(neighbor)}'s current distance with the candidate from ${label(current)}.`, pass, distances, relaxations, neighbor, edgeCost)
                };
                ...
                changed = true;
                relaxations++;
                if (!order.includes(label(neighbor)))
                    order.push(label(neighbor));
                yield {
                    type: "push",
                    graph,
                    current,
                    ...
                    workspace: makeWorkspace(`Distance updated through ${label(current)} on pass ${pass}.`, pass, distances, relaxations, neighbor, edgeCost)
                };
            }
        }
        if (!changed)
