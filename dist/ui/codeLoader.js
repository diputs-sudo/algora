...
export function initCodeLoader(category, algorithmName, options) {
    const { rootSelector, defaultLanguage = "python", moreInfoSelector } = options;
...
export async function copyText(text) {
    if (navigator.clipboard?.writeText) {
        try {
            ...
            // hm .... idk I MEAN this should work so ...
        }
    }
...
function buildPath(language, category, algorithmName) {
    ...
    return new URL(`../../${language}/${category}/${fileName}.${extension}`, window.location.href).href;
}
