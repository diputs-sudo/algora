import { createMetaBinarySearchInitialStep, metaBinarySearch } from "./algorithms/meta_binary.js";
...
document.addEventListener("DOMContentLoaded", () => {
    initLearnMore();
    initCodeLoader("array_search", "meta_binary_search", {
        rootSelector: "#standardCodeSection",
        defaultLanguage: "python",
        moreInfoSelector: "#standardMoreInfo"
    });
    initCodeLoader("array_search", "meta_binary_search_production", {
    ...
    let target = 74;
    let controller = new SearchController(metaBinarySearch(dataset, target), visualizer);
    visualizer.render(createMetaBinarySearchInitialStep(dataset, target));
    ...
    function reset() {
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(metaBinarySearch(dataset, target), createMetaBinarySearchInitialStep(dataset, target));
