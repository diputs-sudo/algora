function createWorkspace(array, target, offset, index, fibM, fibMm1, fibMm2, detail) {
...
function createFibonacciNumbers(length) {
    let fibMm2 = 0;
    let fibMm1 = 1;
...
export function createFibonacciSearchInitialStep(array, target) {
    ...
    return {
        ...
        workspace: array.length > 0 && index !== undefined
            ? createWorkspace(array, target, -1, index, fibM, fibMm1, fibMm2, "Fibonacci Search probes by offset plus the F-2 number, then shrinks the Fibonacci window.")
            : {
                title: "Fibonacci Search Workspace",
...
export function* fibonacciSearch(array, target) {
    ...
    while (fibM > 1) {
        ...
        yield {
            ...
            workspace: createWorkspace(array, target, offset, index, fibM, fibMm1, fibMm2, `Use index = min(offset + F-2, n - 1) = ${index}.`)
        };
        if (value === target) {
            yield {
                type: "found",
                ...
                workspace: createWorkspace(array, target, offset, index, fibM, fibMm1, fibMm2, `Found ${target} at the Fibonacci probe index.`)
            };
            return;
        }
        if (value < target) {
            ...
            yield {
                ...
                workspace: createWorkspace(array, target, offset, Math.min(offset + fibMm2, length - 1), fibM, fibMm1, fibMm2, `Move the offset to ${offset} and shift the Fibonacci window right.`)
            };
        }
        else {
            fibM = fibMm2;
            ...
            yield {
                ...
                workspace: createWorkspace(array, target, offset, Math.min(offset + Math.max(fibMm2, 0), length - 1), fibM, fibMm1, fibMm2, "Drop down two Fibonacci sizes to keep the left portion.")
            };
        }
    }
    ...
    if (fibMm1 === 1 && finalIndex < length) {
        ...
        yield {
            ...
            workspace: createWorkspace(array, target, offset, finalIndex, fibM, fibMm1, fibMm2, "One possible value remains after the Fibonacci window collapses.")
        };
        if (array[finalIndex] === target) {
            yield {
                ...
                workspace: createWorkspace(array, target, offset, finalIndex, fibM, fibMm1, fibMm2, `Found ${target} in the final remaining slot.`)
            };
            return;
        }
    }
