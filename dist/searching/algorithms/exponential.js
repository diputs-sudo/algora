...
function createRangeBuilderInsight(array, target, phase, previousBound, currentBound, discoveredLow, discoveredHigh, comparison, decisionText, note, nextJump) {
...
function createExpansionWorkspace(array, target, bound, detail) {
    return {
        title: "Range Builder",
        detail,
        rows: [
...
export function createExponentialSearchInitialStep(array, target) {
    return {
        type: "narrow",
        array,
        target,
        ...
        rangeSearch: array.length > 0
            ? createRangeBuilderInsight(array, target, "expand", 0, 1, undefined, undefined, `first value ${array[0]}`, "Check index 0 before doubling the bound.", "Exponential Search builds a range quickly, then switches to Binary Search.", 2)
            : undefined,
        workspace: {
            title: "Range Builder",
...
export function* exponentialSearch(array, target) {
    ...
    yield {
        ...
        rangeSearch: createRangeBuilderInsight(array, target, array[0] === target ? "found" : "expand", 0, 1, undefined, undefined, `${array[0]} ${array[0] === target ? "==" : "!="} ${target}`, array[0] === target ? "The first value is the target." : "The first value is not enough, so begin doubling bounds.", "The first index is a cheap early check before exponential growth.", 2),
        workspace: createExpansionWorkspace(array, target, 0, "The first value is checked before range expansion begins.")
    };
    if (array[0] === target) {
        yield {
            type: "found",
    ...
    while (bound < length && array[bound] <= target) {
        ...
        yield {
            ...
            rangeSearch: createRangeBuilderInsight(array, target, "expand", previousBound, bound, undefined, undefined, `${array[bound]} <= ${target}`, `Accept bound ${bound}; double to ${bound * 2}.`, "The probe value is still not past the target, so the range must extend farther right.", bound * 2),
            workspace: createExpansionWorkspace(array, target, bound, `${array[bound]} is less than or equal to ${target}, so the possible range is farther right.`)
        };
        previousBound = bound;
        bound *= 2;
    }
    ...
    yield {
        ...
        rangeSearch: createRangeBuilderInsight(array, target, "binary", low, bound, left, right, bound >= length ? `bound ${bound} is outside array` : `${array[transitionProbe]} > ${target}`, `The discovered range is [${left}, ${right}], so switch to Binary Search.`, "This is the important phase transition: fast expansion becomes exact narrowing."),
        workspace: createBinaryWorkspace(array, target, left, Math.floor((left + right) / 2), right, `The expansion phase found a candidate range from ${left} to ${right}.`)
    };
    while (left <= right) {
        ...
        yield {
            ...
            rangeSearch: createRangeBuilderInsight(array, target, value === target ? "found" : "binary", low, bound, left, right, `${value} ? ${target}`, value === target ? `Return index ${mid}.` : "Use Binary Search rules inside the discovered range.", "The expensive unknown range problem is already solved; this is now ordinary Binary Search."),
            workspace: createBinaryWorkspace(array, target, left, mid, right, `Run Binary Search inside the expanded range.`)
        };
        if (value === target) {
            yield {
                type: "found",
        ...
        if (left <= right) {
            ...
            yield {
                ...
                rangeSearch: createRangeBuilderInsight(array, target, "binary", low, bound, left, right, value < target ? `${value} < ${target}` : `${value} > ${target}`, value < target ? `Discard everything before index ${left}.` : `Discard everything after index ${right}.`, "Binary Search keeps shrinking the range discovered by expansion."),
                workspace: createBinaryWorkspace(array, target, left, nextMid, right, `Continue Binary Search inside the narrowed range.`)
            };
        }
    }
    yield {
