export const defaultFractionalCatalogs = [
    [4, 12, 19, 27, 39, 52, 68],
    [6, 14, 23, 31, 44, 59, 72],
    [3, 16, 25, 36, 48, 61, 80],
    [8, 18, 29, 40, 55, 70, 90]
...
function createWorkspace(catalogs, target, catalogIndex, probeIndex, anchorIndex, detail) {
...
function createInsight(phase, catalogs, catalogIndex, anchorIndex, probeIndex, note, resultIndex, comparison, decisionText) {
    return {
        phase,
        catalogIndex,
...
export function createFractionalCascadingInitialStep(catalogs, target) {
    ...
    return {
        ...
        fractionalCascading: first.length > 0
            ? createInsight("first-search", catalogs, 0, probe, probe, "The first catalog pays the full binary-search cost. Later catalogs reuse this position as an anchor.", undefined, undefined, "Find the first lower bound normally.")
            : undefined,
...
export function* fractionalCascadingSearch(catalogs, target) {
    ...
    for (let catalogIndex = 0; catalogIndex < catalogs.length; catalogIndex++) {
        ...
        const phase = catalogIndex === 0 ? "first-search" : "cascade";
        ...
        yield {
            ...
            fractionalCascading: createInsight(phase, catalogs, catalogIndex, anchor, probe, catalogIndex === 0
                ? "This first search creates the position reused by the cascade."
                : "The bridge gives a near position, so only a local correction is needed in this teaching view.", undefined, `${catalog[probe]} ? ${target}`, catalogIndex === 0 ? "Run full lower-bound search here." : "Adjust locally from the bridged position."),
            workspace: createWorkspace(catalogs, target, catalogIndex, probe, anchor, catalogIndex === 0 ? "Seed the cascade with a normal lower-bound search." : "Reuse the previous result as a bridge.")
        };
        ...
        yield {
            ...
            fractionalCascading: createInsight(found ? "found" : phase, catalogs, catalogIndex, probe, resultProbe, found
                ? "The target was found in this catalog."
                : "Carry this lower-bound position forward to the next catalog.", result, result < catalog.length ? `${catalog[result]} ${found ? "=" : ">="} ${target}` : "past end", found ? `Record catalog ${catalogIndex + 1}, index ${result}.` : `Bridge index ${result} into the next catalog.`),
            workspace: createWorkspace(catalogs, target, catalogIndex, resultProbe, probe, found ? "This catalog has a real match." : "The lower bound becomes the next bridge.")
        };
    }
    yield {
        type: "done",
        ...
        fractionalCascading: createInsight("done", catalogs, Math.max(catalogs.length - 1, 0), anchor, anchor, "All catalogs have been searched using the cascaded lower-bound position.", anchor, "cascade complete", "Any found matches were reported as the cascade advanced."),
        workspace: {
            title: "Fractional Cascading Workspace",
