function highestPowerOfTwoBelow(length) {
    let bit = 1;
    while (bit * 2 < length) {
        bit *= 2;
    }
...
function createWorkspace(array, target, position, bit, testIndex, detail) {
    return {
...
function createBitStates(largestBit, activeBit, keptBits, skippedBits) {
    const bits = [];
...
function createMetaInsight(array, position, activeBit, largestBit, keptBits, skippedBits, decision, note, comparison, decisionText) {
...
function createDecisionText(decision, activeBit) {
    switch (decision) {
        case "keep":
...
export function createMetaBinarySearchInitialStep(array, target) {
    ...
    return {
        ...
        metaBinary: array.length > 0
            ? createMetaInsight(array, -1, bit, bit, keptBits, skippedBits, "pending", `Start with the largest useful bit, +${bit}. If the test value is still smaller than ${target}, that bit becomes part of the base index.`, undefined, "No bit has been decided yet.")
            : undefined,
        workspace: array.length > 0 && testIndex !== undefined
            ? createWorkspace(array, target, -1, bit, testIndex, "Meta Binary Search builds the final lower-bound position with binary lifting.")
            : {
                title: "Meta Binary Search Workspace",
...
export function* metaBinarySearch(array, target) {
    ...
    while (bit > 0) {
        ...
        yield {
            ...
            metaBinary: createMetaInsight(array, position, bit, largestBit, keptBits, skippedBits, inside ? "pending" : "outside", inside
                ? `Test base ${position < 0 ? "before start" : position} plus ${bit}, which lands on index ${testIndex}.`
                : `Base plus ${bit} would land past the array, so this bit cannot be used.`, inside ? `${array[testIndex]} ? ${target}` : `${testIndex} is outside`, inside
                ? "Now compare the test value with the target to decide this bit."
                : `Reject +${bit} because it cannot point to a real array value.`),
            workspace: createWorkspace(array, target, position, bit, testIndex, "If the test value is smaller than the target, keep that bit in the base index.")
        };
        if (inside && array[testIndex] < target) {
            ...
            yield {
                ...
                metaBinary: createMetaInsight(array, position, Math.floor(bit / 2), largestBit, keptBits, skippedBits, "keep", `Keep +${bit}. The base moves to index ${position}, and the next bit checks a smaller jump.`, `${array[position]} < ${target}`, `${array[position]} is smaller than the target, so keep +${bit}.`),
                workspace: createWorkspace(array, target, position, Math.floor(bit / 2), position + Math.floor(bit / 2), "The base moved right. The next smaller bit will test a finer jump.")
            };
        }
        else if (inside) {
            skippedBits.add(bit);
            yield {
                ...
                metaBinary: createMetaInsight(array, position, Math.floor(bit / 2), largestBit, keptBits, skippedBits, "skip", `Skip +${bit}. It would move the base too far, so the base stays ${position < 0 ? "before the array" : `at index ${position}`}.`, `${array[testIndex]} >= ${target}`, `${array[testIndex]} is not smaller than the target, so reject +${bit}.`),
                workspace: createWorkspace(array, target, position, Math.floor(bit / 2), position + Math.floor(bit / 2), "The candidate may be at this index or to its left, so the base stays put.")
            };
        }
        if (!inside) {
            skippedBits.add(bit);
    ...
    if (candidate < length) {
        ...
        yield {
            ...
            metaBinary: createMetaInsight(array, position, 0, largestBit, keptBits, skippedBits, array[candidate] === target ? "found" : "miss", `All bits are decided. The only possible match is base + 1, index ${candidate}.`, array[candidate] === target ? `${array[candidate]} = ${target}` : `${array[candidate]} != ${target}`, array[candidate] === target
        ...
        if (array[candidate] === target) {
            yield {
                ...
                metaBinary: createMetaInsight(array, position, 0, largestBit, keptBits, skippedBits, "found", `The final candidate matches ${target}, so the search returns index ${candidate}.`, `${array[candidate]} = ${target}`, `Return index ${candidate}.`),
                workspace: {
                    title: "Target Found",
    ...
    yield {
        ...
        metaBinary: createMetaInsight(array, position, 0, largestBit, keptBits, skippedBits, "miss", candidate < length
            ? `Index ${candidate} is the lower-bound candidate, but it does not equal ${target}.`
            : `The built base reaches the last smaller value, so ${target} would be inserted at index ${candidate}.`, candidate < length ? `${array[candidate]} != ${target}` : `candidate ${candidate} is outside`, candidate < length
