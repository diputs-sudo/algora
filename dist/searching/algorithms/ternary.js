function createWorkspace(array, target, left, mid1, mid2, right, detail) {
...
function getSplitPoints(left, right) {
    const third = Math.floor((right - left) / 3);
    return [left + third, right - third];
}
export function createTernarySearchInitialStep(array, target) {
    ...
    return {
        ...
        workspace: array.length > 0 && mid1 !== undefined && mid2 !== undefined
            ? createWorkspace(array, target, 0, mid1, mid2, right, "Ternary Search compares the target against two split points before choosing one third to keep.")
            : {
                title: "Ternary Search Workspace",
...
export function* ternarySearch(array, target) {
    ...
    while (left <= right) {
        ...
        yield {
            ...
            workspace: createWorkspace(array, target, left, mid1, mid2, right, `The split points are index ${mid1} and index ${mid2}.`)
        };
        ...
        if (left <= right) {
            ...
            yield {
                ...
                workspace: createWorkspace(array, target, left, nextMid1, nextMid2, right, "Discard the thirds that cannot contain the target.")
            };
        }
    }
    yield {
