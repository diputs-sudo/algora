import { createUniformBinarySearchInitialStep, uniformBinarySearch } from "./algorithms/uniform_binary.js";
...
document.addEventListener("DOMContentLoaded", () => {
    initLearnMore();
    initCodeLoader("array_search", "uniform_binary_search", {
        rootSelector: "#standardCodeSection",
        defaultLanguage: "python",
        moreInfoSelector: "#standardMoreInfo"
    });
    initCodeLoader("array_search", "uniform_binary_search_production", {
        rootSelector: "#productionCodeSection",
        defaultLanguage: "python"
    });
    const visualizer = new SearchVisualizer("searchGraphContainer");
    let dataset = [4, 10, 16, 23, 31, 39, 47, 58, 66, 79, 91, 104, 118];
    let target = 79;
    let controller = new SearchController(uniformBinarySearch(dataset, target), visualizer);
    visualizer.render(createUniformBinarySearchInitialStep(dataset, target));
    ...
    function reset() {
        ...
        if (dataset.length === 0) {
            dataset = [4, 10, 16, 23, 31, 39, 47, 58, 66, 79, 91, 104, 118];
        }
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(uniformBinarySearch(dataset, target), createUniformBinarySearchInitialStep(dataset, target));
