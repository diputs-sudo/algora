import { createSentinelLinearSearchInitialStep, sentinelLinearSearch } from "./algorithms/sentinel_linear.js";
...
document.addEventListener("DOMContentLoaded", () => {
    initLearnMore();
    initCodeLoader("array_search", "sentinel_linear_search", {
        rootSelector: "#standardCodeSection",
        defaultLanguage: "python",
        moreInfoSelector: "#standardMoreInfo"
    });
    initCodeLoader("array_search", "sentinel_linear_search_production", {
    ...
    let target = 55;
    let controller = new SearchController(sentinelLinearSearch(dataset, target), visualizer);
    visualizer.render(createSentinelLinearSearchInitialStep(dataset, target));
    ...
    function reset() {
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(sentinelLinearSearch(dataset, target), createSentinelLinearSearchInitialStep(dataset, target));
