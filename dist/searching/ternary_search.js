import { createTernarySearchInitialStep, ternarySearch } from "./algorithms/ternary.js";
...
document.addEventListener("DOMContentLoaded", () => {
    initLearnMore();
    initCodeLoader("array_search", "ternary_search", {
        rootSelector: "#standardCodeSection",
        defaultLanguage: "python",
        moreInfoSelector: "#standardMoreInfo"
    });
    initCodeLoader("array_search", "ternary_search_production", {
        rootSelector: "#productionCodeSection",
        defaultLanguage: "python"
    });
    const visualizer = new SearchVisualizer("searchGraphContainer");
    let dataset = [3, 8, 13, 21, 29, 34, 42, 55, 67, 72, 88, 96, 111];
    let target = 67;
    let controller = new SearchController(ternarySearch(dataset, target), visualizer);
    visualizer.render(createTernarySearchInitialStep(dataset, target));
    ...
    function reset() {
        ...
        if (dataset.length === 0) {
            dataset = [3, 8, 13, 21, 29, 34, 42, 55, 67, 72, 88, 96, 111];
        }
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(ternarySearch(dataset, target), createTernarySearchInitialStep(dataset, target));
