import { createJumpSearchInitialStep, jumpSearch } from "./algorithms/jump.js";
...
document.addEventListener("DOMContentLoaded", () => {
    initLearnMore();
    initCodeLoader("array_search", "jump_search", {
        rootSelector: "#standardCodeSection",
        defaultLanguage: "python",
        moreInfoSelector: "#standardMoreInfo"
    });
    initCodeLoader("array_search", "jump_search_production", {
        rootSelector: "#productionCodeSection",
        defaultLanguage: "python"
    });
    const visualizer = new SearchVisualizer("searchGraphContainer");
    let dataset = [2, 7, 12, 18, 23, 31, 39, 44, 52, 61, 73, 86, 94];
    let target = 52;
    let controller = new SearchController(jumpSearch(dataset, target), visualizer);
    visualizer.render(createJumpSearchInitialStep(dataset, target));
    ...
    function reset() {
        ...
        if (dataset.length === 0) {
            dataset = [2, 7, 12, 18, 23, 31, 39, 44, 52, 61, 73, 86, 94];
        }
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(jumpSearch(dataset, target), createJumpSearchInitialStep(dataset, target));
