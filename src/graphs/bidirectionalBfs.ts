...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function reset() {
        ...
        if (Number.isFinite(selectedIndex) && sampleGrids[selectedIndex]) graph = sampleGrids[selectedIndex];
        controller.reset(bidirectionalBfs(graph), createBidirectionalBfsInitialStep(graph));
        controller.setSpeed(Number(speedRange.value));
    }
    ...
    editModeSelect.addEventListener("change", () => visualizer.setEditMode(editModeSelect.value as GridEditMode));
