...
function stepDistances(distances: Map<string, number>): Record<string, number> {
    return Object.fromEntries(distances.entries());
}

export function createZeroOneBfsInitialStep(graph: GraphData): GraphStep {
    return {
        type: "start",
        graph,
        current: graph.start,
...
export function* zeroOneBfs(graph: GraphData): Generator<GraphStep> {
    const distances = new Map<string, number>([[key(graph.start), 0]]);
    const parent = new Map<string, string>();
    const settled = new Set<string>();
    const order: string[] = [];
    const deque: GridPoint[] = [graph.start];
    ...
    while (deque.length > 0) {
        ...
        yield {
            ...
            workspace: makeWorkspace(
                `${label(current)} leaves the deque front and becomes settled.`,
                deque,
                distances,
                order
            )
        ...
        for (const neighbor of neighborsFor(graph, current)) {
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `Try ${label(neighbor)} with candidate distance ${nextDistance}.`,
                    deque,
                    distances,
                    order,
                    neighbor,
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `${label(neighbor)} moves to the deque ${weight === 0 ? "front" : "back"} because its edge weight is ${weight}.`,
                    deque,
                    distances,
                    order,
                    neighbor,
