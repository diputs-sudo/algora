...
export function createBfsInitialStep(graph: GraphData): GraphStep {
    return {
        type: "start",
        graph,
        current: graph.start,
...
export function* breadthFirstSearch(graph: GraphData): Generator<GraphStep> {
    const queue: GridPoint[] = [graph.start];
    const visited = new Set<string>([key(graph.start)]);
    const parent = new Map<string, string>();
    const distances = new Map<string, number>([[key(graph.start), 0]]);
    const order: string[] = [];
    ...
    while (queue.length > 0) {
        ...
        yield {
            ...
            workspace: makeWorkspace(
                `${label(current)} leaves the queue front. BFS now checks its next layer of neighbors.`,
                queue,
                visited,
                order,
                distances
        ...
        for (const neighbor of neighborsFor(graph, current)) {
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `Check whether ${label(neighbor)} has already been discovered.`,
                    queue,
                    visited,
                    order,
                    distances,
            ...
            if (visited.has(neighborKey)) {
                yield {
                    ...
                    workspace: makeWorkspace(
                        `${label(neighbor)} already has its shortest known layer distance.`,
                        queue,
                        visited,
                        order,
                        distances,
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `${label(neighbor)} joins the back of the queue for a later layer.`,
                    queue,
                    visited,
                    order,
                    distances,
