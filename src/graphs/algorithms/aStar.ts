...
function heuristic(point: GridPoint, target: GridPoint): number {
...
function scoreValues(
    ...
    function makeWorkspace(
        ...
        current?: GridPoint): GraphWorkspaceState {
            return {
                title: "A* Workspace",
                detail,
                rows: [
        ...
        function scoreRecord(scores: Map<string, number>): Record<string, number> {
            return Object.fromEntries(scores.entries());
        }

        export function createAStarInitialStep(graph: GraphData, heuristicWeight = 1): GraphStep {
        ...
        export function* aStar(graph: GraphData, heuristicWeight = 1): Generator<GraphStep> {
            heuristicWeight = normalizeWeight(heuristicWeight);
            const startKey = key(graph.start);
            const targetKey = key(graph.target);
            const gScore = new Map<string, number>([[startKey, 0]]);
            const fScore = new Map<string, number>([[startKey, heuristic(graph.start, graph.target) * heuristicWeight]]);
            ...
            while (open.length > 0) {
                open.sort((left, right) => {
                    ...
                    if (fDifference !== 0) return fDifference;
                ...
                yield {
                    ...
                    workspace: makeWorkspace(
                        `${label(current)} moves from the open set to the closed set.`,
                        open,
                        Array.from(closed),
                        gScore,
                        fScore,
                    ...
                    for (const neighbor of neighborsFor(graph, current)) {
                        ...
                        yield {
                            ...
                            workspace: makeWorkspace(
                                `Compare the candidate g=${tentativeG} with ${label(neighbor)}'s current g=${currentG === Infinity ? "infinity" : currentG}.`,
                                open,
                                Array.from(closed),
                                gScore,
                                fScore,
                            ...
                            if (closed.has(neighborKey) || tentativeG >= currentG) continue;
                            ...
                            yield {
                                ...
                                workspace: makeWorkspace(
                                    `${label(neighbor)} is now scored with g=${tentativeG}, h=${h}, and f=${tentativeG + weightedH}.`,
                                    open,
                                    Array.from(closed),
                                    gScore,
                                    fScore,
