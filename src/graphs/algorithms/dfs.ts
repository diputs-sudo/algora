...
function wallSetFor(graph: GraphData): Set<string> {
    return new Set(graph.walls.map(key));
}

function isInside(graph: GraphData, point: GridPoint): boolean {
    return point.row >= 0
...
export function createDfsInitialStep(graph: GraphData): GraphStep {
    return {
        type: "start",
        graph,
        current: graph.start,
...
export function* depthFirstSearch(graph: GraphData): Generator<GraphStep> {
    const visited = new Set<string>();
    const parent = new Map<string, string>();
    const order: string[] = [];
    const stack: GridPoint[] = [graph.start];
    ...
    while (stack.length > 0) {
        ...
        if (visited.has(currentKey)) {
            yield {
                ...
                workspace: makeWorkspace(
                    `${label(current)} is already visited. Continue with the next stack cell.`,
                    visited,
                    stack,
                    order
                )
        ...
        yield {
            ...
            workspace: makeWorkspace(
                `${label(current)} becomes visited. DFS now checks nearby open cells.`,
                visited,
                stack,
                order
            )
        ...
        for (const neighbor of neighbors) {
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `Check whether ${label(neighbor)} is open and unvisited.`,
                    visited,
                    stack,
                    order,
                    neighbor
            ...
            yield {
                ...
                workspace: makeWorkspace(
                    `${label(neighbor)} is open and unvisited, so DFS adds it to the stack.`,
                    visited,
                    stack,
                    order,
                    neighbor
        ...
        yield {
            ...
            workspace: makeWorkspace(
                `All available neighbors of ${label(current)} have been checked.`,
                visited,
                stack,
                order
            )
