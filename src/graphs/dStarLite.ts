...
import { GraphData, GraphStep, GridPoint } from "./visualizer/types.js";
...
type PlannerState = "editing" | "planning" | "ready" | "repairing";
...
function pathCost(step: GraphStep): number | null {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const expandedMetric = document.getElementById("expandedMetric") as HTMLElement;
    const replannedMetric = document.getElementById("replannedMetric") as HTMLElement;
    const pathCostMetric = document.getElementById("pathCostMetric") as HTMLElement;
    const replansMetric = document.getElementById("replansMetric") as HTMLElement;

    let baseGraph = cloneGraph(baseGrids[0]);
    let graph = cloneGraph(baseGraph);
    let planner: DStarLitePlanner;
    let controller: GraphController;
    let plannerState: PlannerState = "editing";
    ...
    function initializePlanner(clear = false) {
        graph = { ...cloneGraph(baseGraph), heuristic: heuristicSelect.value as GraphData["heuristic"] };
    ...
    function renderMetrics(step: GraphStep) {
        ...
        if (step.type === "done") plannerState = "ready";
    }
    ...
    function repairWall(point: GridPoint) {
        if (!editingEnabled) return;
        if (key(point) === key(graph.start) || key(point) === key(graph.target)) return;
    ...
    stepBtn.addEventListener("click", () => {
        if (plannerState === "editing") plannerState = "planning";
        controller.step();
    });
