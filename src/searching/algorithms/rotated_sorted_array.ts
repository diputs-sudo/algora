import { RotatedArraySearchInsight, SearchStep, SearchWorkspaceState } from "../visualizer/types.js";
...
function createInsight(
    ...
    mid: number,
    high: number,
    phase: RotatedArraySearchInsight["phase"],
    sortedSide: RotatedArraySearchInsight["sortedSide"],
    keepSide: RotatedArraySearchInsight["keepSide"],
    comparison: string,
    decisionText: string,
    note: string
): RotatedArraySearchInsight {
    const leftRange = low <= mid ? [low, mid] as [number, number] : undefined;
    const rightRange = mid <= high ? [mid, high] as [number, number] : undefined;
...
export function createRotatedSortedArraySearchInitialStep(array: number[], target: number): SearchStep {
    ...
    return {
        ...
        rotatedArray: array.length > 0
            ? createInsight(
                ...
                "unknown",
                "unknown",
                `${array[0]} ? ${array[mid]} ? ${array[high]}`,
                "Find which half is sorted before deciding which half can be discarded.",
                "A rotated sorted array is still two sorted runs joined at the rotation point."
            )
            : undefined,
...
export function* rotatedSortedArraySearch(array: number[], target: number): Generator<SearchStep> {
    let low = 0;
    ...
    while (low <= high) {
        ...
        yield {
            ...
            rotatedArray: createInsight(
                ...
                high,
                "identify",
                lowValue <= midValue ? "left" : "right",
                "unknown",
                lowValue <= midValue ? `${lowValue} <= ${midValue}` : `${midValue} <= ${highValue}`,
                lowValue <= midValue ? "The left half is sorted." : "The right half is sorted.",
                "Rotated binary search works because one side still has normal sorted order."
            ),
        ...
        if (midValue === target) {
            yield {
                ...
                rotatedArray: createInsight(
                    ...
                    `${midValue} == ${target}`,
                    `Return index ${mid}.`,
                    "The middle value matches, so the search stops immediately."
                ),
        ...
        if (lowValue <= midValue) {
            if (lowValue <= target && target < midValue) {
                ...
                yield {
                    ...
                    rotatedArray: createInsight(
                        ...
                        "left",
                        "left",
                        `${lowValue} <= ${target} < ${midValue}`,
                        `Keep indices ${low} through ${high}.`,
                        "When the target falls inside the sorted half's value range, the other half can be discarded."
                    ),
                ...
                yield {
                    ...
                    rotatedArray: createInsight(
                        ...
                        "left",
                        "right",
                        `${target} not in [${lowValue}, ${midValue})`,
                        low <= high ? `Keep indices ${low} through ${high}.` : "The candidate window is empty.",
                        "If the target cannot fit in the sorted half, it must be in the rotated half or absent."
                    ),
            ...
            yield {
                ...
                rotatedArray: createInsight(
                    ...
                    "right",
                    "right",
                    `${midValue} < ${target} <= ${highValue}`,
                    `Keep indices ${low} through ${high}.`,
                    "The target fits the sorted right half's value range."
                ),
            ...
            yield {
                ...
                rotatedArray: createInsight(
                    ...
                    "right",
                    "left",
                    `${target} not in (${midValue}, ${highValue}]`,
                    low <= high ? `Keep indices ${low} through ${high}.` : "The candidate window is empty.",
                    "The sorted right half cannot contain the target, so search the other half."
                ),
