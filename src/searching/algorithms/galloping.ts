...
export function createGallopingSearchInitialStep(array: number[], target: number): SearchStep {
    return {
        type: "narrow",
        array,
        target,
...
export function* gallopingSearch(array: number[], target: number): Generator<SearchStep> {
    ...
    while (jump < length && array[jump] < target) {
        ...
        yield {
            ...
            workspace: createGallopWorkspace(
                ...
                previous,
                jump,
                "The target must be farther right than the current jump point."
            )
        };
    ...
    yield {
        ...
        workspace: createBinaryWorkspace(
            ...
            high,
            "The final search is ordinary Binary Search inside the bracketed range."
        )
    };
