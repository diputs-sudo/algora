...
function normalizeRotatedSortedArray(values: number[]): number[] {
...
function generateRotatedSortedArray(size: number): number[] {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function reset() {
        ...
        datasetInput.value = dataset.join(",");
        controller.reset(
            rotatedSortedArraySearch(dataset, target),
            createRotatedSortedArraySearchInitialStep(dataset, target)
        );
