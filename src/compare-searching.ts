...
import { SearchStep } from "./searching/visualizer/types.js";
...
function getCodeFileName(algorithm: AlgorithmKey, language: CompareLanguage): string {
...
async function loadCode(slot: CompareSlot) {
    ...
    const path = new URL(
        `../../${slot.language}/array_search/${getCodeFileName(slot.algorithm, slot.language)}.${extensions[slot.language]}`,
...
function getCompatibilityWarning(firstAlgorithm: AlgorithmKey, secondAlgorithm: AlgorithmKey): string {
...
function getDatasetForAlgorithm(algorithm: AlgorithmKey, values: number[]): number[] {
    return algorithmInfo[algorithm].inputShape === "sorted" ? sortedUnique(values) : [...values];
}

function getDatasetForComparison(firstAlgorithm: AlgorithmKey, secondAlgorithm: AlgorithmKey, values: number[]): number[] {
    const requiresSortedInput =
        algorithmInfo[firstAlgorithm].inputShape === "sorted" ||
        algorithmInfo[secondAlgorithm].inputShape === "sorted";

    return requiresSortedInput ? sortedUnique(values) : [...values];
}

function getTargetForAlgorithm(algorithm: AlgorithmKey, dataset: number[], rawTarget: number): number {
...
function createGenerator(algorithm: AlgorithmKey, dataset: number[], target: number): Generator<SearchStep> {
    switch (algorithm) {
        case "linear_search":
...
function createInitialStep(algorithm: AlgorithmKey, dataset: number[], target: number): SearchStep {
    switch (algorithm) {
        case "linear_search":
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const targetInput = document.getElementById("compareTargetInput") as HTMLInputElement;
    ...
    const warning = document.getElementById("compareSearchWarning") as HTMLElement;
    const warningText = document.getElementById("compareSearchWarningText") as HTMLElement;
    ...
    function resetControllers() {
        ...
        recomputeInputs();
        controllerOne.reset(
            createGenerator(slotOne.algorithm, datasetOne, targetOne),
            createInitialStep(slotOne.algorithm, datasetOne, targetOne)
        );
        controllerTwo.reset(
            createGenerator(slotTwo.algorithm, datasetTwo, targetTwo),
            createInitialStep(slotTwo.algorithm, datasetTwo, targetTwo)
        );
